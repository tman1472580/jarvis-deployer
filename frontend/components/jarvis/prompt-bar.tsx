...
export function PromptBar({ selectedTask, onSend }: PromptBarProps) {
  const [message, setMessage] = useState("")
  const [isFocused, setIsFocused] = useState(false)
  const inputRef = useRef<HTMLInputElement>(null)

  // Auto-focus input when a task is selected
  useEffect(() => {
    ...
    <div className="relative">
      ...
      >
        ...
        {isFocused && (
          <div className="absolute inset-0 rounded-sm pointer-events-none animate-pulse-glow"
            style={{
              boxShadow: "0 0 30px oklch(0.7 0.15 195 / 0.15)"
            }}
          />
        )}
      </div>
      ...
      <div className="flex justify-between items-center mt-2 px-2">
        ...
        <div className="flex items-center gap-3">
          <span className="text-[9px] font-mono text-cyan-600/40">CTRL+ENTER TO SEND</span>
          <div className="flex gap-0.5">
            {Array.from({ length: 5 }).map((_, i) => (
              <div 
                key={i} 
                className={`w-1 h-2 ${i < 4 ? "bg-cyan-500/60" : "bg-cyan-800/40"}`}
              />
            ))}
          </div>
        </div>
